...
// argv carries no configuration — only which of the two things the binary does
// — and an argument it does not have is refused rather than ignored (SPEC 8.3).
...
if (invocation.mode === "check-drift") {
  process.exit(driftExitCode(await checkCategoryDrift()));
}
