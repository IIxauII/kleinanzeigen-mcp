...
**The npm package is `kanzeigen-mcp`, and that is not the project's name.** `kleinanzeigen-mcp` on npm belongs to somebody else — an unrelated MCP server over the same site, published 2026-09-01 under `io.github.taneron/kleinanzeigen`. It is a real package rather than a squat, so the name is not recoverable and a dispute was not attempted; the first `npm publish` under it would 403, and `npx -y kleinanzeigen-mcp` fetches that server rather than this one. The clip is §4.6's, already worn by every slug the plugin owns (§8.8).

**Two identities are spelled differently from here on, and only the distribution one moved.** The **distribution** name is `kanzeigen-mcp`: `package.json`'s `name` and `bin`, `server.json`'s npm identifier, the plugin's pin, the MCPB manifest name and its release-asset filename. The **project** name stays `kleinanzeigen-mcp`: the repository and every URL derived from it, `serverInfo.name` (§3), and — load-bearing — **§8.5's User-Agent token, which does not move**. It is a literal in `src/user-agent.ts` rather than a read of `package.json`, so the rename cannot reach it by accident, and it must not reach it on purpose: the identify half is what a site operator writes a block rule against, and ADR-0003's non-circumvention argument rests on it. `mcpName: io.github.IIxauII/kleinanzeigen` is untouched too — the registry authenticates the namespace against the GitHub account and reads `mcpName` out of npm metadata, and neither depends on the package name.
...
**Every slug the plugin owns takes §4.6's trademark clip** — plugin `kanzeigen`, marketplace `kanzeigen`, skill directory `searching-kanzeigen` — and so, since the rename in §8.3, does the npm name `kanzeigen-mcp`. The discoverability cost of the clip is accepted across all four; what is no longer paid is the cost of the slugs and the package disagreeing with each other. The clip stops at prose: the skill's **description must contain "kleinanzeigen.de"**, because that string is what makes it trigger, and naming the site you read is descriptive use. `plugin.json` takes §4.6's one description string, which is now shared across five slots.
