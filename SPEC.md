...
**And a third constraint of the same family is neither ordering nor configuration but time.** npm's read-after-write is not immediate: the publish returns before `registry.npmjs.org/<pkg>/<version>` exists, and the registry step runs seconds later in the same `publish` lifecycle — 3.4 seconds on the 1.0.1 release, against a document that first answered 200 two to three minutes later ([#84](https://github.com/IIxauII/kleinanzeigen-mcp/issues/84)). Plugin order sequences the two calls and says nothing about the gap, so **the registry step blocks on the version document being readable** before it stamps or submits, bounded and non-zero on expiry so a genuine publish failure still reddens the run; and the submission itself mints a registry token per attempt and retries the propagation 400 — that one narrowly-matched failure and nothing else, because every other way it fails is permanent. The token is per-attempt because the registry issues five-minute ones and the wait may outlast several of them; the workflow's own `login` step is a pre-flight that fails a broken OIDC context before the drift gate runs, not the credential the submission uses. The reason this is worth a gate rather than a re-run is that it fails on the **last** step of a release, after npm, the tag, the release and the commit-back have all landed: a re-dispatch finds no new commits and never reaches the registry again, and recovery is by hand ([`docs/maintenance.md`](./docs/maintenance.md)).
