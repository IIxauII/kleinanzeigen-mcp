...
describe("the marketplace", () => {
  ...
  it("takes the trademark clip in its own slug and in the plugin's", () => {
    // §4.6's clip governs every slug the project owns downstream, and the npm
    // name `kanzeigen-mcp` takes it too, so these match it (SPEC 8.8).
...
describe("the plugin's .mcp.json", () => {
  it("pins the server version exactly, never floating", () => {
    ...
    expect(mcp.mcpServers.kleinanzeigen.args).toEqual(["-y", `${pkg.name}@${pkg.version}`]);
  });
