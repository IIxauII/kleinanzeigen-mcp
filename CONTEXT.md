...
_Avoid_: Staleness, dataset rot

**Category drift** _(Kategoriedrift)_:
Dataset drift in the *category tree*: a node added or removed since the dataset was generated.
...
**Location drift** _(Ortsdrift)_:
*Dataset drift* in the city dataset: a locality id added or removed, **or a locality renamed**. The rename case is why the drift check walks `/s-katalog-orte.html` rather than the cities sitemap alone — the sitemap carries ids and no names, so a sitemap-only diff would be blind to it ([ADR-0004](./docs/adr/0004-the-city-dataset-has-two-sources.md)).
_Avoid_: City drift, place drift
